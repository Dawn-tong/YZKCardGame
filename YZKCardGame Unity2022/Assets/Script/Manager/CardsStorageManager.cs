...
public static class CardsStorageManager {
	...
	static Card[] BuildSerializableCards() {
		Card[] cardsList = currentPlayerCardManager.cardsList;
		if (cardsList == null || cardsList.Length != CardManager.MAX_TOTAL_CARDS) {
			cardsList = new Card[CardManager.MAX_TOTAL_CARDS];
		}
		Card[] serialized = new Card[CardManager.MAX_TOTAL_CARDS];
		for (int i = 0; i < CardManager.MAX_TOTAL_CARDS; i++) {
	...
	public static void ResetToPresetCards() {
		Card[] presetInstance = PresetCardsList.GetPresetCardsList(CurrentCardSetIndex);
		ApplyLoadedCards(presetInstance);
		currentPlayerCardManager.NotifyCardLevelChanged();
	}
}

...
public class CameraDragManager : ManagerBase<CameraDragManager> {
	...
	/// 设置当前场景的摄像机拖拽控制器
	/// </summary>
	public void SetCurrentController(Vector2 leftDownEdge, Vector2 rightUpEdge, float minZoom = 2f, float maxZoom = 20f) {
		if (currentController != null) {
			currentController.SetBounds(leftDownEdge, rightUpEdge);
			currentController.SetZoomRange(minZoom, maxZoom);
		}
	}
	public void SetCurrentController(Vector2 leftDownEdge, Vector2 rightUpEdge) {
		if (currentController != null) {
			currentController.SetBounds(leftDownEdge, rightUpEdge);
		}
	}
	public void SetCurrentController(float minZoom = 2f, float maxZoom = 20f) {
		if (currentController != null) {
			currentController.SetZoomRange(minZoom, maxZoom);
		}
	}

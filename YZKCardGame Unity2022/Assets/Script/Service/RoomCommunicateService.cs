...
public class RoomCommunicateService {
	static RoomCommunicateService instance;
	public static RoomCommunicateService Instance {
		get {
			if (instance == null)
				instance = new RoomCommunicateService();
			return instance;
		}
	}
	...
	void OnPlayerJoinGameRequest(ulong senderID, PlayerJoinGameRequest request) {
		...
		if(receivedCardsCount == PlayerManager.Instance.GetPlayerCount()) {
			...
			GameSceneService.Instance.ChooseBoardPosition();
		}
	}
	...
	void SendAllPlayersCardsListToAllPlayers() {
		Log.IncreasePerfixLength();
		Debug.Log($"{Log.perfix}————        通知所有玩家        ————");
		Debug.Log($"{Log.perfix}消息内容:所有玩家卡组获取完毕，开始游戏");
		UIMessagePanel.Instance.AddMessage($"通知所有玩家，所有玩家卡组获取完毕，开始游戏");
		NetMessage message = new NetMessage();
		message.Response = new NetMessageResponse();
		...
		foreach (Player p in PlayerManager.Instance.allPlayers) {
			...
			playerInfo.cardsList.SetCardInfoList(p.cardManager.cardsArray);
	...
	void OnReadyToStartResponse(ulong senderID, ReadyToStartResponse response) {
		...
		message.Request.playerJoinGame.player.cardsList.SetCardInfoList(PlayerManager.Instance.currentPlayer.cardManager.cardsArray);
		NetManager.Instance.SendMessageToServer(message);
	}

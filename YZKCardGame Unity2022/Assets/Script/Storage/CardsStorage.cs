	...
	static void ApplyLoadedCards(Card[] source) {
		...
		for (int i = 0; i < CardsListManager.MAX_TOTAL_CARDS; i++) {
			...
			if (card == null || !card.exists || card.index < 0) {
				...
				cardsList[i] = card;
				if(cardsList[i].cardType == CardType.Normal) {
					if(!currentPlayerCardManager.IsValidNormalCard(cardsList[i])) {
						currentPlayerCardManager.ResetNormalCardToDefault(cardsList[i]);
					}
				}

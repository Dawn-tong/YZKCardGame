...
public class CardSettingUI_OneCardPanel : MonoBehaviour {
	...
	void DeleteCard() {
		...
		if(cardManager.DeleteCard(cardInfo.CardIndex)) {
			...
			propretiesPanel.SetActive(false);
			UpdateButtonState();
			cardSettingBoard.DeleteBoardChess(positionX, positionY);
		}
	}
	...
	public event CloseDelegate CloseCallback;
	public void ClickButtonToReturn() {
		HideOneCardsPanel();
	...
	public void ClickButtonToPutCardOnBoard() {
		...
		cardSettingBoard.OnTileClicked -= ClickBoardToOpenOneCardPanel;
		//操作UI则取消监听
		allCardsPanel.ShowCallback += CancelListenerPutEvent;
	...
	void PutCardOnBoard(int positionX, int positionY) {
		...
		cardSettingBoard.PutCardOnBoard(cardInfo.Card, positionX, positionY);
		//取消监听放置事件
	...
	public void ClickBoardToOpenOneCardPanel(int positionX, int positionY) {
		Card card = cardManager.FindCardByPosition(positionX, positionY);
		if (card == null) {
			return;
		}
		ShowOneCardsPanel();
		UpdateOneCardPanel(card.index);
	}
}
